									...
									BOOL *pRowFullSized) const
{
	...
	for(int i = nFirstToReview; TRUE; i += nIncrement )
	{
		if( i < 0 || i >= nTotalDockCount )
			break;
									   ...
									   DWORD nMode)
{
	...
	for(int i = nReviewStart; i<=nReviewStop; i++)
	{
		CControlBar *pBar = (CControlBar *) m_pDockBar->m_arrBars[i];
...
LRESULT CTControlBar::NcButtons_HitTest( CPoint point,
									  CTBarNcAreaButton **ppBtn)
{
	...
	return (LRESULT)nFirstHT;
}

LRESULT CTControlBar::OnNcHitTest( CPoint point)
{
	LRESULT nHT = NcButtons_HitTest(point);
			...
			if( nCountInRowBefore > 0 && nSpaceNeededBefore > 0 )
			{
				...
				if( nSpaceToCompress > 0 )
				{
					...
					if( nSpaceAvail > 0 )
					{
						...
						for(int i = nCountInRowBefore; i>0; i--)
						{
