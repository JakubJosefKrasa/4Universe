...
void CTachyonHUGEMAP::InitVB( CD3DDevice *pDevice)
{
	...
	for ( auto i =0; i<m_nDiffuseCount; i++)
	{
		...
		m_vBLENDTILE.push_back(new WORD[nLength]);

		memset( LPDWORD(m_vDIFFUSEBUF.back()), 0x00000000, nCount * sizeof(DWORD));
		memset( LPWORD(m_vBLENDTILE.back()), 0xFFFF, nLength * sizeof(WORD));
	}
	InitForNULLTILE();
...
void CTachyonHUGEMAP::InitForNULLTILE()
{
	...
	memset( LPWORD(m_vBLENDTILE[0]), 0xFFFF, nLength * sizeof(WORD));
}
									 ...
									 BYTE bCullView)
{
	...
	for( int i=0; i<INT(m_vBLOCK.size()); i++)
		if((!bCullView || BLOCKInVIEW( m_vBLOCK[i], fDIR, fRANGE)) && IsValidBLOCK( m_vBLOCK[i], pCENTER))
		{
			...
			WORD wTILE = TILE_NULL;

			if( m_vBLOCK[i]->Width() < 2 )
			{
				m_vBLOCK[i]->m_wTILE = FindDetailID(
					vPOINT.m_nX,
					vPOINT.m_nY);
				wTILE = m_vBLOCK[i]->m_wTILE == TILE_NULL ? TILE_NULL : m_vBLOCK[i]->m_wTILE & TILE_MASK;

				if( wTILE >= INT(m_vDETAILDATA.size()) || !m_vDETAILDATA[wTILE] )
				{
					m_vBLOCK[i]->m_wTILE = TILE_NULL;
					wTILE = TILE_NULL;
				}
			}

			if( pTILE->find(wTILE) == pTILE->end() )
				pTILE->insert( MAPDWORD::value_type( wTILE, 0));
								 ...
								 LPIPOINT pCENTER)
{
	...
	if(!pTILE->empty())
	{
		...
		VECTORBLOCK vBLEND[TILE_MAX];
		DWORD vSTART[TILE_MAX];
		DWORD vINDEX[TILE_MAX];
		...
		for ( auto i =0; i<m_nBlockCount; i++)
		{
			...
			DWORD dwIndex = vINDEX[pRECT->m_wTILE == TILE_NULL ? (TILE_MAX-1) : (pRECT->m_wTILE) & TILE_MASK];
			...
			if( nWIDTH > 1 || bREGION != BLANK_REGION )
			{
				...
				if( nWIDTH < 2 )
				{
					...
					if( bINDEX != 0xFF )
					{
						...
						{
							WORD wTILE = ((LPWORD) m_vBLENDTILE[j])[bINDEX * nCount * nCount + vPOINT.m_nY * nCount + vPOINT.m_nX];

							if( wTILE != TILE_NULL && pTILE->find(wTILE) != pTILE->end() )
							{
								m_vBLEND[vINDEX[wTILE]] += INT(pRECT->m_vPOLY.size()) / 3;
								vBLEND[vINDEX[wTILE]].push_back(pRECT);
							}
						}
					}
				}
								 ...
								 LPIPOINT pCENTER)
{
	...
	if(!pTILE->empty())
	{
		...
		VECTORBLOCK vBLEND[9][TILE_MAX];
		DWORD vSTART[9][TILE_MAX];
		DWORD vCOUNT[9][TILE_MAX];
		...
		if( pTILE->find(TILE_NULL) != pTILE->end() )
		{
			...
			for ( auto i =0; i<9; i++)
			{
				m_vBLEND.push_back(0);
				m_vCOUNT.push_back(0);
				vCOUNT[i][TILE_MAX-1] = m_vCOUNT.size() - 1;
			}
		}
		...
		for ( auto i =0; i<m_nBlockCount; i++)
		{
			...
			if( nWIDTH > 1 || bREGION != BLANK_REGION )
			{
				WORD wTILE = pRECT->m_wTILE == TILE_NULL ? (TILE_MAX-1) : pRECT->m_wTILE & TILE_MASK;
				...
				m_vCOUNT[vCOUNT[bINDEX][wTILE]] += INT(pRECT->m_vPOLY.size()) / 3;
				if( nWIDTH < 2 )
				{
					nCellX += m_nCellX;
					nCellZ += m_nCellZ;
					...
					for( int j=0; j<INT(m_vBLENDTILE.size()); j++)
					{
						wTILE = ((LPWORD) m_vBLENDTILE[j])[bINDEX * nCount * nCount + vPOINT.m_nY * nCount + vPOINT.m_nX];

						if( wTILE != TILE_NULL && pTILE->find(wTILE) != pTILE->end() )
						{
							m_vBLEND[vCOUNT[bINDEX][wTILE]] += INT(pRECT->m_vPOLY.size()) / 3;
							vBLEND[bINDEX][vINDEX[wTILE]].push_back(pRECT);
						}
					}
				}
			}
		...
		for ( auto i =0; i<m_nBlockCount; i++)
		{
			...
			if( nWIDTH > 1 || bREGION != BLANK_REGION )
			{
				...
				for ( auto j =0; j<INT(pRECT->m_vPOLY.size()); j++)
				{
					pINDEX[vSTART[bINDEX][vINDEX[wTILE]]] = (WORD) vPOINT[pRECT->m_vPOLY[j]];
					vSTART[bINDEX][vINDEX[wTILE]]++;
				}
...
void CTachyonHUGEMAP::FreeDIFFUSE( BYTE bINDEX)
{
	...
	for( itDIFFUSE = m_mapDIFFUSEINDEX.begin(); itDIFFUSE != m_mapDIFFUSEINDEX.end(); itDIFFUSE++)
		wTileID[(*itDIFFUSE).second] = WORD((*itDIFFUSE).first);
	nCount = 9 * nCount * nCount;
	...
	for( BYTE i=1; i<BYTE(m_vDIFFUSELOCK.size()); i++)
		if(m_vDIFFUSELOCK[i])
		{
			...
			if(!m_vDIFFUSELOCK[i])
			{
				itDIFFUSE = m_mapDIFFUSEINDEX.find(wTileID[i]);

				if( itDIFFUSE != m_mapDIFFUSEINDEX.end() )
					m_mapDIFFUSEINDEX.erase(itDIFFUSE);

				memset( LPWORD(m_vBLENDTILE[i]), 0xFFFF, nCount * sizeof(WORD));
			}
		}
}
...
WORD CTachyonHUGEMAP::FindDetailID( int nTileX,
								    int nTileZ)
{
	int nUnitX = nTileX / m_nUnitLength - m_nUnitX;
	int nUnitZ = nTileZ / m_nUnitLength - m_nUnitZ;
...
BYTE CTachyonHUGEMAP::FindFreeDIFFUSE( LPDIRECT3DDEVICE9 pDevice)
{
	...
	for( BYTE i=1; i<BYTE(m_vDIFFUSELOCK.size()); i++)
		if(!m_vDIFFUSELOCK[i])
		{
			...
			memset( LPWORD(m_vBLENDTILE[i]), 0xFFFF, nLength * sizeof(WORD));
									...
									int nCellZ)
{
	...
	for( int i=1; i<INT(m_vDIFFUSEBUF.size()); i++)
		if(m_vDIFFUSELOCK[i])
		{
			...
			memset( &LPWORD(m_vBLENDTILE[i])[m_bINDEX[bCELL] * nCount * nCount], 0xFFFF, nCount * nCount * sizeof(WORD));
		}
		...
		for( int j=0; j<nCount; j++)
		{
			...
			if( wTileID != TILE_NULL )
			{
				...
				if(!(wTileID & 0x8000))
					for( byte k=0; k<8; k++)
					{
						WORD wBLEND = FindDetailID(
							nCellX + j + nBLEND[k][0],
							nCellZ + i + nBLEND[k][1]);

						if( !(wBLEND & 0x8000) && (wBLEND & TILE_MASK) > (wTileID & TILE_MASK) )
						{
							BlendTILE(
								pDevice,
								&IPOINT(
						...
						else if( wBLEND == TILE_NULL || (wBLEND & TILE_MASK) < (wTileID & TILE_MASK) )
						{
							BlendTILE(
								pDevice,
								&IPOINT(
								 ...
								 WORD wTileID)
{
	...
	{
		MAPDWORD::iterator itTILE = m_mapDIFFUSEINDEX.find(DWORD(wTileID & TILE_MASK));
		...
		if( itTILE != m_mapDIFFUSEINDEX.end() )
		{
			...
			(LPWORD(m_vBLENDTILE[(*itTILE).second]))[m_bINDEX[bCELL] * nLength * nLength + nLocalZ * nLength + nLocalX] = WORD(wTileID & TILE_MASK);
		}
	}
}
...
LPDWORD CTachyonHUGEMAP::GetDIFFUSEBUF( LPDIRECT3DDEVICE9 pDevice,
										...
										WORD wTileID)
{
	MAPDWORD::iterator finder = m_mapDIFFUSEINDEX.find(wTileID);
	...
	if( finder == m_mapDIFFUSEINDEX.end() )
	{
		...
		m_mapDIFFUSEINDEX.insert( MAPDWORD::value_type( wTileID, bINDEX));
	}
	else
								...
								int nUnitID)
{
	...
	// DETAIL INDEX.
	pUNIT->m_vDETAIL.clear();
	nSIZE = *((int *) pDATA);
	pDATA += sizeof(int);

	int size = nSIZE / sizeof(WORD);
	for( int i=0; i<size; ++i )
	{
		WORD wDETAIL = 0;
		memcpy( &wDETAIL, pDATA, sizeof(WORD));
		pDATA += sizeof(WORD);

		pUNIT->m_vDETAIL.push_back( wDETAIL );
	}
	// DETAIL INDEX.
