...
void CTachyonObject::RenderSILHOUETTE( CD3DDevice *pDevice, CD3DCamera *pCamera, DWORD dwColor, BOOL bChild )
{
	...
	DWORD dwCOLOR = dwColor; //D3DCOLOR_XRGB(255,0,0);
	...
	D3DXVECTOR3 vOffset[4] =
	{
		...
		D3DXVECTOR3(-0.010f, 0.0f, 0.0f),
		D3DXVECTOR3(0.0f, -0.010f, 0.0f),
		D3DXVECTOR3(0.0f, 0.010f, 0.0f),
	};

	for( int m=0 ; m < 4 ; ++m )
	{
		D3DXMATRIX mView(pCamera->m_matView);
		D3DXMATRIX mOffset;
		D3DXMatrixTranslation(&mOffset, vOffset[m].x, vOffset[m].y, vOffset[m].z );
		mView *= mOffset;
		...
		for( itOBJ = m_mapEQUIP.begin(); itOBJ != m_mapEQUIP.end(); itOBJ++)
		{
			...
			if(pEQUIP)
			{
				...
				pEQUIP->m_bAlpha = 255;
				pEQUIP->m_bLOD = m_bLOD;
				pEQUIP->RenderSILHOUETTE( pDevice, pCamera, dwColor, TRUE);
				pEQUIP->m_vPosition = vPosition;
			}
		}
	}
	...
	if( !bChild )
	{
		pDevice->m_pDevice->SetTransform(D3DTS_VIEW, &(pCamera->m_matView) );
...
void CTachyonObject::Render( CD3DDevice *pDevice, CD3DCamera *pCamera)
{
	...
	for( it = m_OBJ.m_mapDRAW.begin(); it != m_OBJ.m_mapDRAW.end(); it++)
	{
		...
		for( int i=0; i<INT(pDRAW->size()); i++)
		{
			...
			pDevice->m_pDevice->SetRenderState( D3DRS_ZWRITEENABLE, CTachyonMesh::m_gZWriteable && m_bZWrite && pPART->m_pTEX->m_bZWrite);
			pDevice->m_pDevice->SetRenderState( D3DRS_ZENABLE,  CTachyonMesh::m_gZEnable && m_bZEnable && pPART->m_pTEX->m_bZEnable);
