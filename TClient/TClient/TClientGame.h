...
public:
	...
	static void GetSecondToRemainTime( DWORD dwSecond, DWORD* dwDay, DWORD* dwHour, DWORD* dwMin, DWORD* dwSec);
...
protected:
	// Path node guide
	CTClientObjBase m_vTNODEGUIDE[TNODEGUIDE_COUNT];
	D3DXVECTOR3 m_vTNODEPATH[TNODEGUIDE_COUNT + 1];

	LPTNPCPORTAL m_pTNODEPORTAL;
	LPTTERMPOS m_pTNODEPATH;

	// Auto Path Target
	D3DXVECTOR3 m_vTAUTOPATH;
	D3DXVECTOR3 m_vTAUTOPREV;
	TTERMPOS m_vTAUTOTARGET;
	...
	LPTNPCPORTAL m_pTAUTOPORTAL;
	LPTTERMPOS m_pTAUTOPATH;
	D3DXPLANE m_vTAUTOPLANE;
...
public:
	BOOL m_bChallengeGame;
	BYTE m_bChallengeStatus;
	BYTE m_bPrevChallengeStatus;

	void ResetDisplayChallengeInfo();
	bool IsMiniGameBattle()
	{
		return m_bTournamentBattle || m_bChallengeGame;
	}
	...
	BOOL m_bRPSGaming;
#endif
	CString m_strMeetingInviter;
	DWORD m_dwRemainSecondChangeAID;

	CString m_strAssistantInvitor;
	VTTASSISTANT m_vTASSISTANT;
	void ClearTASSITANT();
	void SortTASSITANT();

	void SetChannel( BYTE bNewChannel );
	...
	void DropItem( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint point );
	...
	void DropItem_MainUIToDefault( UINT nFlags, BYTE bTargetUI, BYTE bSlotID, CPoint point );
	...
	D3DXVECTOR2 GetTNODEGOAL(
		LPTTERMPOS pTNODEPATH,
		LPTNPCPORTAL pTNODEPORTAL);

	BYTE GetTNODELOCALPORTAL(
		LPTNPCPORTAL *pTRESULT,
	...
	LPTNPCPORTAL FindTNODEPORTAL(
		LPVTNPCPORTAL pTPORTAL,
		LPD3DXVECTOR3 pFROM,
		LPTTERMPOS pTO);
	...
	void DoTAUTOPATH( LPTTERMPOS pTAUTOPATH);
	void DoDEFACTION();
	void DoCHAT();
	...
	void FindSlaveRecallByHost( DWORD dwHostID, LPVTRECALL );
	void FindRecallByHost( DWORD dwHostID, LPVTRECALL );
	...
	D3DXVECTOR3 GetTAUTOPATHDIR();
	D3DXVECTOR3 GetTHOSTDIR();
	...
	void CalcTNODEGUIDE( DWORD dwTick);
...
public:
	...
	int OnGM_ITEM_BUY();
#ifdef MAGICITEMSHOP
	int OnGM_MAGICITEM_BUY();
#endif
	int OnGM_PORTAL_REQ();
	int OnGM_PORTAL_REQ_DIRECT();
	int OnGM_GETMONEY();
	int OnGM_GETALL();
	...
	int OnGM_QUEST_MAPOPEN();
	int OnGM_QUEST_MAPCLOSE();
	...
	int OnGM_GUILD_REQ();
	int OnGM_ASSISTANT_REQ();
	int OnGM_ASSISTANT_OK();
	int OnGM_ASSISTANT_NO();
	int OnGM_ASSISTANT_BUSY();
	int OnGM_ASSISTANT_KICK();
	int OnGM_ASSISTANT_SELFKICK();

	int OnGM_SELECT_COUNTRY_DLG();

	int OnGM_TACTICS_REQ();
	int OnGM_SHOW_INVITE_TACTICS();
	int OnGM_GUILD_EXIT_WARN();
	int OnGM_GUILD_EXIT();
	int OnGM_GUILD_TACTICS_SELF_QUIT();
	...
	int OnGM_GUILD_DONATE_PVPPOINT();
...
//	int OnGM_TEXTOOL_COMPO();
...
#endif
	int OnGM_SELECT_DO_AID();
	int OnGM_SELECT_AID_D();
	int OnGM_SELECT_AID_C();
	int OnGM_NPC_MEETING();
	int OnGM_NPC_MERCENARY();
	int OnGM_NPC_NATIONALITY1();
	int OnGM_NPC_NATIONALITY2();
	int OnGM_DO_CHANGE_TO_BROIR();
	int OnGM_REALLY_SELECT_COUNTRY();
	int OnGM_SELECT_MY_COUNTRY();
	int OnGM_SELECT_MY_COUNTRY_D();
	int OnGM_SELECT_MY_COUNTRY_C();
	int OnGM_DO_SELECT_COUNTRY();
	int OnGM_ANSWER_MEETING_IGNORE();
	int OnGM_ANSWER_METTING_OK();
	int OnGM_ANSWER_MEETING_NO();
	int OnGM_INVITE_MEETING_ROOM();
	int OnGM_CHALLENGE_JOIN();
	int OnGM_CHALLENGE_JOIN_DO();
	int OnGM_CHALLENGE_GIVEUP();
	int OnGM_CHALLENGE_GIVEUP_DO();
	int OnGM_CHALLENGE_EVENT();
	int OnGM_NPC_DUEL3();
	int OnGM_NPC_DUEL7();
	int OnGM_SELECT_AID_GIVEUP();
	int OnGM_SELECT_AID_GIVEUP_DO();
	int OnGM_CHANGE_ORG_COUNTRY();
	int OnGM_NODE_TELEPORT_YES();
	int OnGM_NODE_TELEPORT_NO();
	int OnGM_TUTORIAL_START();
	int OnGM_TUTORIAL_SKIP();
...
#ifdef MAGICITEMSHOP
	int OnCS_MAGICITEMBUY_ACK( CTachyonSession *pSession, CPacket *pPacket);
#endif
...
#ifdef ADD_SKYGARDEN
	int OnCS_ENTERSKYGARDEN_ACK( CTachyonSession *pSession, CPacket *pPacket);
#endif
	...
	int OnCS_WARCOUNTRYBALANCE_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_MEETINGROOM_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ARENA_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ARENATEAM_ACK( CTachyonSession *pSession, CPacket *pPacket );

#ifdef ADD_SKYGARDEN
	int OnCS_ASSISTANT_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ASSISTANTRESULT_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ASSISTANTADD_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ASSISTANTDEL_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ASSISTANTATTR_ACK( CTachyonSession *pSession, CPacket *pPacket );
	int OnCS_ASSISTANTSTAT_ACK( CTachyonSession *pSession, CPacket *pPacket );
#endif
