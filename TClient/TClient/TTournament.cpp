...
#endif
{
	...
	m_pENTRY_CHARGE4 = FindKid(ID_CTRLINST_ENTRY_CHARGE4);
	...
	m_pENTRY_CHARGE4_VALUE = FindKid(ID_CTRLINST_ENTRY_CHARGE4_VALUE);
		...
		case TT_ENTRY:
			{
				...
				m_pENTRY_CHARGE4->ShowComponent(TRUE);
				...
				m_pENTRY_CHARGE4_VALUE->ShowComponent(TRUE);
		...
		case TT_JOIN:
			{
				...
				m_pENTRY_CHARGE4->ShowComponent(FALSE);
				...
				m_pENTRY_CHARGE4_VALUE->ShowComponent(FALSE);
...
void CTTournament::OnSelectMatch( TMATCH* pMATCH )
{
		...
		case TT_JOIN:
			{
				...
				m_pENTRY_CHARGE4_VALUE->m_strText = CTChart::Format( TSTR_TOURNAMENT_LIMIT_LEVEL, pMATCH->m_bMinLevel, pMATCH->m_bMaxLevel );
	...
	else
	{
		...
		m_pENTRY_CHARGE4_VALUE->m_strText.Empty();
...
void CTTournament::OnNotify( DWORD from, WORD msg, LPVOID param )
{
	if( msg == TNM_SEL_CHANGE )
	{
		if( from == m_pMATCHLIST->m_id )
		{
			...
			if( nCur != -1 )
			{
				DWORD dwITEM = m_pMATCHLIST->GetItemData( nCur, 0 );
				if( dwITEM != (DWORD)(-1) )
				{
