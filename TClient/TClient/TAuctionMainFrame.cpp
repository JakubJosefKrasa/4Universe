...
:	CTFrameGroupBase( pParent, pDesc, ID_CTRLINST_INNERPOS )
{
	...
	for ( auto i = 1; i < MAX_ITEM_SLOT; ++i )
	{
...
void CTAuctionMainFrame::SetSelect(UINT nFrame)
{
	CTClientGame* pTGAME = CTClientGame::GetInstance();
	if( nFrame == 1 &&
		pTGAME->GetTargetObj() &&
		pTGAME->GetTargetObj()->m_bContryID != pTGAME->GetMainChar()->m_bContryID )
	{
		m_FrameInfoArray[ nFrame ].m_pTabCtrl->Select( (BOOL) FALSE );
		return ;
	}

	CTFrameGroupBase::SetSelect( nFrame );
}
...
BOOL	CTAuctionMainFrame::AvailableBID()
{
	CTClientGame* pGame = CTClientGame::GetInstance();
	BYTE bAuctionCountry = pGame->GetTargetObj()->m_bContryID;
	BYTE bMyCountry = pGame->GetMainChar()->m_bContryID;
	BYTE bMyAidCountry = pGame->GetMainChar()->m_bAidCountryID;
	
	switch( bAuctionCountry )
	{
	case TCONTRY_D:
		if( bMyCountry == TCONTRY_D || (bMyCountry == TCONTRY_B && bMyAidCountry == TCONTRY_D ) )
			return TRUE;

	case TCONTRY_C:
		if( bMyCountry == TCONTRY_C || (bMyCountry == TCONTRY_B && bMyAidCountry == TCONTRY_C ) )
			return TRUE;
...
void	CTAuctionMainFrame::SetSearchFrame()
{
	if( !AvailableBID() )
	{
		...
		m_pBtn[TBUTTON_THIRD]->ShowComponent( FALSE );
		return ;
	}
