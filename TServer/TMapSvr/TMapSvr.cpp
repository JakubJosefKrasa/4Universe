...
DWORD CTMapSvrModule::LoadConfig()
{
	...
	strRegKey.Format(_T("SYSTEM\\CurrentControlSet\\Services\\TMAP_GSP\\Config"));
...
DWORD CTMapSvrModule::CreateThreads()
{
	...
	std::cout << "Creating _ControlThread" << endl;
	...
	std::cout << "Creating _BatchThread" << endl;
	...
	std::cout << "Creating _DBThread" << endl;
	m_hDB = CreateThread(
		NULL, 0,
		_DBThread,
		(LPVOID) this,
	...
	std::cout << "Creating _AIThread" << endl;
	m_hAI = CreateThread(
		NULL, 0,
		_AIThread,
		(LPVOID) this,
	...
	std::cout << "Creating _LogThread" << endl;
	m_hLog = CreateThread(
		NULL, 0,
		_LogThread,
		(LPVOID) this,
	...
	{
		std::cout << "Creating _WorkThread[" << (int)i << "]" << endl;
...
BYTE CTMapSvrModule::Accept()
{
	...
	*/
	...
	if (!hIocpWork || !pPlayer->WaitForMessage() || !pPlayer || inet_ntoa(pPlayer->m_addr.sin_addr) == NULL)
	{
		pPlayer->Close();
		delete pPlayer;
...
DWORD CTMapSvrModule::BatchThread()
{
	...
	while(TRUE)
	{
		...
		else
		{
			...
			{
				LogEvent("Batch Close CloseSession dwResult = %d, PacketID: %d\n", dwResult, pBUF->m_packet.GetID());
				std::cout << "Batch Close CloseSession dwResult" << dwResult << ", PacketID" << pBUF->m_packet.GetID() << endl;
...
DWORD CTMapSvrModule::OnReceive( LPPACKETBUF pBUF)
{
	...
	switch(wMsgID)
	{
		if (pBUF->m_pSESSION)
		{
			...
			ON_RECEIVE(CT_USERKICKOUT_ACK)
			ON_RECEIVE(CT_USERMOVE_ACK)
			ON_RECEIVE(CT_MONSPAWNFIND_ACK)
...
BYTE CTMapSvrModule::MonMoneyTake(CTPlayer * pPlayer, DWORD dwMonID)
{
	...
	case OWNER_PARTY	:
		if( pMon->m_dwKeeperID == pPlayer->GetPartyID() )
		{
			...
			for(DWORD i=0; i<DWORD(vParty.size()); i++)
			{
...
void CTMapSvrModule::LocalReward()
{
	...
	for(auto ll=0; ll<m_vOccupation.size(); ll++)
	{
		LPTLOCAL pLocal = m_vOccupation[ll];
